...
- `docs/`: handwritten project notes. Includes `docs/creadits.md` and short agent follow-up notes in `docs/agent_handover.md`.
...
- Player locomotion uses composition: `Player` owns public facing/animation API, while child `PlayerMovementController` owns input snapshots, shared timers, and movement state switching. Movement tuning lives in `PlayerMovementConfig`; concrete velocity flow lives in child `PlayerMovementState` nodes, with common movement helpers on the base state. Player ground adhesion uses configured `CharacterBody2D.floor_snap_length` to keep downhill slopes smooth. Future directional gameplay should read `Player.facing_direction`, not sprite flip state.
- Reusable level scenes should use a `Node2D` root that extends `LevelTileMap`; root `tile_map_layers` exports every `TileMapLayer` that shared tile metadata queries should scan. Climb zones use TileSet custom data `climbable`, not physics collision; climbable tiles must remain non-solid. `LevelTileMap` caches multi-cell atlas tile footprints on `_ready()` and resolves climb queries from that cache. Level subclasses that override `_ready()` must call `super._ready()` so the cache builds. If climbable tiles change at runtime, call `rebuild_climb_cache()`. Player climb movement centers on the climbable tile, top contact stops upward climb movement without detaching, floor contact detaches from climbing unless the player is actively climbing upward, and horizontal movement input detaches from climbing with normal coyote jump timing.
