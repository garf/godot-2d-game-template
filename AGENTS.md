...
- Reusable level tilemaps should extend `LevelTileMap` for shared tile metadata queries. Climb zones use TileSet custom data `climbable`, not physics collision; climbable tiles must remain non-solid. `LevelTileMap` resolves multi-cell atlas tile footprints from the placed tile origin, player climb movement centers on the climbable tile, top contact stops upward climb movement without detaching, floor contact detaches from climbing unless the player is actively climbing upward, and horizontal movement input detaches from climbing with normal coyote jump timing.
