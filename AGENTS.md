...
- Player locomotion uses composition: `Player` owns public facing/animation API, while child `PlayerMovementController` owns input snapshots, shared timers, and movement state switching. Movement tuning lives in `PlayerMovementConfig`; concrete velocity flow lives in child `PlayerMovementState` nodes, with common movement helpers on the base state. Future directional gameplay should read `Player.facing_direction`, not sprite flip state.
